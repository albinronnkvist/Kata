...
public static class TakeATenMinutesWalk
{
    ...
    {
        if (walk.Length is not 10)
        {
            return false;
        }

        var northSouth = walk.Count(d => d is "n") - walk.Count(d => d is "s");
        var eastWest = walk.Count(d => d is "e") - walk.Count(d => d is "w");

        return northSouth is 0 && eastWest is 0;
    }
}
